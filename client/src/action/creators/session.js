...
export const loginUser = (loginData) => {
    return async function(dispatch) {
        try {
            ...
            const response = await Axios.post(`${API_BASE_URL}${API_LOGIN_ENDPOINT}`, loginData);
            dispatch(loggedInUser({...response.data}));
            notify(`Welcome back, ${response.data.user.username}!`, dispatch);
        } catch(e) {
            if (e.response.status === 401) {
                dispatch({type: AUTH_ERROR, message: 'Incorrect Username or Password'});
            } else {
...
export const registerUser = (registerData) => {
    return async function(dispatch) {
        try {
            dispatch({type: AUTH_REQUEST});
            const response = await Axios.post(`${API_BASE_URL}${API_REGISTER_ENDPOINT}`, registerData);
            dispatch(loggedInUser(response.data));
...
export const refreshSessionUser = () => {
    return async function(dispatch, getState) {
        try {
            ...
            const response = await Axios.get(`${API_BASE_URL}${API_USER_ENDPOINT}/${id}`);
...
export const createPlaceRating = (rating, place_id) => {
    return async function(dispatch, getState) {
        try {
            ...
            const response = await Axios.post(`${API_BASE_URL}/user/${userId}/rating/place/${place_id}`, {rating});
            dispatch({type: PLACE_CREATE_RATING_SUCCESS, payload: response.data.place_rating});
        } catch(e) {
            console.log(e);
        }
    }
...
export const updatePlaceRating = (newRating, oldRating, id) => {
    return async function(dispatch, getState) {
        try {
            dispatch({type: AUTH_REQUEST});
            const userId = getState().sessionState.user.id;
            const response = await Axios.patch(`${API_BASE_URL}/user/${userId}/rating/place/${id}`, {rating: newRating})
...
export const deletePlaceRating = (rating, id) => {
    return async function(dispatch, getState) {
        try {
            dispatch({type: AUTH_REQUEST});
            const userId = getState().sessionState.user.id;
            const response = await Axios.delete(`${API_BASE_URL}/user/${userId}/rating/place/${id}`);
            dispatch({type: PLACE_DELETE_RATING_SUCCESS, payload: {place_rating: rating, place_id: id}});
        } catch(e) {
            console.log(e);
        }
    };
...
export const createDrinkRating = (rating, id) => {
    return async function(dispatch, getState) {
        try {
            dispatch({type: AUTH_REQUEST});
            const userId = getState().sessionState.user.id;
            const response = await Axios.post(`${API_BASE_URL}${API_USER_ENDPOINT}/${userId}/rating/drink/${id}`, {rating});
            dispatch({type: DRINK_CREATE_RATING_SUCCESS, payload: response.data.drink_rating});
        } catch(e) {
...
export const updateDrinkRating = (rating, oldRating, id) => {
    return async function(dispatch, getState) {
        try {
            dispatch({type: AUTH_REQUEST});
            const userId = getState().sessionState.user.id;
            const response =  await Axios.patch(`${API_BASE_URL}${API_USER_ENDPOINT}/${userId}/rating/drink/${id}`, {rating});
            dispatch({type: DRINK_UPDATE_RATING_SUCCESS, payload: {...response.data.drink_rating, old_rating: oldRating}});
        } catch(e) {
...
export const deleteDrinkRating = (oldRating, id) => {
    return async function(dispatch, getState) {
        try {
            dispatch({type: AUTH_REQUEST});
            const userId = getState().sessionState.user.id;
            await Axios.delete(`${API_BASE_URL}${API_USER_ENDPOINT}/${userId}/rating/drink/${id}`);
            dispatch({type: DRINK_DELETE_RATING_SUCCESS, payload: {old_rating: oldRating, drink_id: id}});
        } catch(e) {
...
function refreshedUser(user) {
    ...
    return {type: REFRESH_SUCCESS, payload: {user: {...user, places_owned: placesOwned, place_ratings: placeRatings, drink_ratings: drinkRatings}, places, drinks }};
}
