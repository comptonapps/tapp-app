...
export const getPlaces = () => {
    return async function(dispatch, getState) {
        try {
            ...
            const response = await Axios.get(`${API_BASE_URL}${API_PLACE_ENDPOINT}?city=${preferredLocation.city}&state=${preferredLocation.state}&page=${page+1}&limit=${limit}`);
...
export const createPlace = (placeData, completion) => {
    return async function(dispatch, getState) {
        try {
            ...
            dispatch({type: PLACES_REQUEST});
            const response = await Axios.post(`${API_BASE_URL}/user/${userId}/place`, placeData);
...
function createdPlace(data) {
    ...
    data.place.rating = 0;
    return {type: PLACES_CREATE_SUCCESS, payload: {place: data.place, place_owner: data.place_owner}}
}
...
export const updatePlace = (placeId, placeData, completion) => {
    return async function(dispatch, getState) {
        try {
            const userId = getState().sessionState.user.id;
            dispatch({type: PLACES_REQUEST});
            const response = await Axios.patch(`${API_BASE_URL}${API_PLACE_ENDPOINT}/${placeId}`, placeData);
