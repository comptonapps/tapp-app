...
function Place() {
    ...
    const getPlaceData = async () => {
        try {
            const response = await Axios.get(`${API_BASE_URL}${API_PLACE_ENDPOINT}/${id}`);
            const place = response.data.place;
            console.log(place)
            const drinks = place.drinks;
            delete place.drinks;
            dispatch({type: 'PLACES_INSERT_RECORD', payload: {[place.id]: place}});
    ...
    return (
        <Page cls="Place">
            {place && 
                <PageHeader>
                    <div className="Place-data">
                        <h1>{place.name}</h1>
                        <h3>{place.address}</h3>
                        <h3>{place.city}, {place.state} {place.zip}</h3>
                    ...
                    <div className="Place-control">
                        ...
                        {place && <button onClick={toggleMap} class='control-btn'>Show Map</button>}
                        {placesOwned[id] && <Link to={`/places/${place.id}/edit`} class='control-btn'>Edit {`${place.name}`}</Link>}
                    </div>
                </PageHeader>
            }
            ...
            {isOwnedBySessionUser() && <button onClick={() => history.push(`/places/${id}/edit`)}>Edit Place</button>}
