...
function Dashboard() {
    ...
    const {place_ratings, drink_ratings, places_owned} = useSelector(st => st.sessionState.user);
    ...
    console.log(drinksRated.map(dr => {
    ...
    return (
        ...
        <div>
            ...
            <PlaceList dashboard={true} places={placesRated.map(pr => {
        ...
        <div>
            ...
            <DrinkList dashboard={true} drinks={drinksRated.map(dr => {
                const drink = drinks[dr.drink_id];
                if (drink) {
                    drink.rating = dr.rating;
                    return drink;
        ...
        <div>
            ...
            { (placesOwned && placesOwned.length) ? 
                <PlaceList hoverable={false} places={placesOwned.map(po => {
