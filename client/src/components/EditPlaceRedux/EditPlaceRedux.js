...
function EditPlaceRedux() {
    ...
    const [showConfirmDraughtChange, setShowConfirmDraughtChange] = useState(false);
    ...
    const getDraughtData = () => {
        return async function(dispatch) {
            try {
                ...
                const response = await Axios.get(`${API_BASE_URL}${API_USER_ENDPOINT}/${userId}${API_PLACE_ENDPOINT}/${id}`);
    ...
    return (
        <Page cls="EditPlaceRedux">
            {place && 
                <>
                {showAddDraughtModal && <SearchUntappd toggle={toggleAddDraughtModal} place_id={id} />}
                {showEditPlaceForm && <EditPlaceModal place={place} toggle={toggleEditPlaceForm}/>}
                <div className="PlaceInfo">
                    ...
                    <button onClick={toggleEditPlaceForm}>Edit {place.name} Info</button>
                </div>
                
                <SectionTitle title="EDIT DRAUGHT LIST">
                    <button onClick={toggleAddDraughtModal}>Add New Draught</button>
                </SectionTitle>
                <EditDraughtList draughts={Object.values(draughts) || []} place_id={id}/>
                </>
            }
        </Page>
    );
