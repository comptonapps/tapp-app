...
function Places() {
    ...
    return (
        <Page cls="Places">
            <PageTitle title="places" />
            <LocationWidget />
            <SearchBar placeholder="Search Places" clearResults={clearResults} handleSearch={handleSearch} />
            <PlaceList places={searchResults.length > 0 ? <h1>Search Results Detected</h1> : sortedResults.map(id => places[id])} />
        </Page>
    )
}
