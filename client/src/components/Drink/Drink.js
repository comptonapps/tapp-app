...
function Drink() {
    ...
    const drink = useSelector(st => st.drinkState.sortedResults[`drink${id}`] || st.drinkState.drinks[id]);
    ...
    const getDrinkData = async () => {
        try {
            ...
            const response = await Axios.get(`${API_BASE_URL}${API_DRINK_ENDPOINT}/${id}?city=${city}&state=${state}`);
