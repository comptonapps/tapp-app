...
function EditDraughtCell({draught, place_id}) {
    ...
    const changeDraughtStatus = (id, active) => {
        return async function(dispatch) {
            try {
                ...
                const response = await Axios.patch(`http://localhost:3003/place/${place_id}/draught/drink/${id}`, {active: active});
    ...
    const deleteDraught = (place_id, drink_id) => {
        return async function(dispatch) {
            try {
                dispatch({type: EDIT_PLACE_REQUEST});
                const response = await Axios.delete(`${API_BASE_URL}${API_PLACE_ENDPOINT}/${place_id}/draught${API_DRINK_ENDPOINT}/${drink_id}`);
    ...
    return (
        <Cell className="edit">
            ...
            <div className="EditDraughtCell-control">
                <button onClick={() => handleStatusClick(draught.drink_id, !draught.active)}>{draught.active ? "Pull Offline" : "Put Online"}</button>
                <button onClick={() => handleDeleteClick(draught.drink_id)}>DELETE</button>
            </div>
        </Cell>
    )
}
