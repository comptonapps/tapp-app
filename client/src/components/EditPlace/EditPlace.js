...
function EditPlace() {
    ...
    const getDraughtData = async () => {
        const response = await Axios.get(`http://localhost:3003/user/1/place/${place_id}`);
    ...
    return (
        <Page cls="EditPlace">
            ...
            { place && 
                <>
                {showAddDraughtModal && <SearchUntappd toggle={toggleAddDraughtModal} place_id={place_id} />}
