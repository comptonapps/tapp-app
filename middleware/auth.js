...
function authenticateJWT(req, res, next) {
  try {
    const { token } =
      req.body || req.headers.authorizaton.replace(/^[Bb]earer /, "").trim();
    if (token) {
      res.locals.user = JWT.validateJWT(token);
      delete req.body.token;
    } else {
      const headerToken = req.headers.authorization;
      res.locals.user = JWT.validateJWT(
        headerToken.replace(/^[Bb]earer /, "").trim()
      );
    }
    return next();
  } catch (e) {
...
function checkForCorrectUserOrAdmin(req, res, next) {
  ...
  if (
    ...
    (res.locals.user.is_admin ||
      (+res.locals.user.id === +user_id ||
        +res.locals.user.id === +req.body.user_id))
  ) {
    return next();
  }
